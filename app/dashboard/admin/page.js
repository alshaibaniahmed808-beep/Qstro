...
export default async function AdminDashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12">
      ...
      <section>
        <h2 className="text-xl font-bold mb-4 text-red-800">Pending Car Approvals</h2>
        <div className="bg-white rounded-xl shadow-sm border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr><th className="p-3 text-left">Title</th><th>Owner</th><th>Action</th></tr>
            </thead>
            <tbody>
              {pendingCars?.map(car => (
                <tr key={car.id} className="border-t">
                  <td className="p-3">{car.title}</td>
                  <td>{car.users?.name}</td>
                  <td>
                    <form action={approveCar.bind(null, car.id)}>
      ...
      <section>
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr><th>User</th><th>Amount</th><th>Proof</th><th>Action</th></tr>
            </thead>
            <tbody>
              {payments?.map(p => (
                <tr key={p.id} className="border-t">
                  <td className="p-3">{p.users?.name}</td>
                  <td>${p.amount}</td>
                  <td><a href={p.proof_image} target="_blank" className="text-primary-600 underline">View</a></td>
                  <td>
