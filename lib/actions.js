...
export async function approvePayment(paymentId) {
  ...
  if (payment) {
    ...
    const planByAmount = { 20: 'starter', 40: 'pro', 80: 'premium' }
    await supabase.from('subscriptions').upsert({
      user_id: payment.user_id,
      plan: planByAmount[Number(payment.amount)] || 'starter',
...
export async function updateUserRole(userId, formData) {
  ...
  const { data: profile } = await supabase.from('users').select('role').eq('id', user.id).single()
  if (profile?.role !== 'admin') throw new Error('Unauthorized')
  if (!['user', 'dealer', 'admin'].includes(role)) throw new Error('Invalid role')
