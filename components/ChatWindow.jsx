...
export default function ChatWindow({ carId, otherUserId }) {
  ...
  const sendMessage = async () => {
    ...
    const { error } = await supabase.from('messages').insert({
      car_id: carId, sender_id: user.id, receiver_id: otherUserId, message: newMsg.trim()
    })
    if (!error) setNewMsg('')
  }

  if (!user) return <p className="text-center py-12">Please log in to chat.</p>
  return (
    <div className="flex flex-col h-[85vh] max-w-3xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="p-4 border-t bg-white flex gap-3 items-center">
        <input
          className="flex-1 border border-gray-200 rounded-full px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
